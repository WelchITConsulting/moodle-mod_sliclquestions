...
class sliclquestions_question {
    ...
    /**
     * Position in the sliclquestions
     * @var int $position
     */
     public $position    = 0;
    ...
    private function insert_response_bool($rid) {
        ...
        if (!empty($val)) { // If "no answer" then choice is empty (CONTRIB-846).
            ...
            return $DB->insert_record('sliclquestions_'.$this->response_table, $record);
        } else {
            return false;
        }
    }
    ...
    private function insert_response_date($rid) {
        global $DB;
        $val = optional_param('q'.$this->id, '', PARAM_CLEAN);
        $checkdateresult = sliclquestions_check_date($val);
        $thisdate = $val;
        if (substr($checkdateresult, 0, 5) == 'wrong') {
            return false;
        }
        // Now use ISO date formatting.
        $checkdateresult = sliclquestions_check_date($thisdate, $insert = true);
        $record = new Object();
        $record->response_id = $rid;
        $record->question_id = $this->id;
            ...
            foreach ($this->choices as $cid => $choice) {
                if (strpos($choice->content, '!other') === 0) {
                    ...
                    if (preg_match("/[^ \t\n]/", $other)) {
                        ...
                        $resid = $DB->insert_record('sliclquestions_response_other', $record);
                        $val = $cid;
                        break;
                    }
                }
    ...
    private function insert_resp_multiple($rid) {
        ...
        foreach ($val as $cid) {
            ...
            if ($cid != 0) { // Do not save response if choice is empty.
                ...
                $record->question_id = $this->id;
                $record->choice_id = $cid;
                $resid = $DB->insert_record('sliclquestions_'.$this->response_table, $record);
            }
        }
        return $resid;
    }
    ...
    private function insert_response_rank($rid) {
        ...
        if ($this->type_id == QUESRATE) {
            ...
            $r = $val;
            if ($val == get_string('notapplicable', 'sliclquestions')) {
                $rank = -1;
            } else {
                $rank = intval($val);
            }
    ...
    private function get_response_bool_results($rids=false) {
        ...
        $sql = 'SELECT choice_id, COUNT(response_id) AS num '.
               'FROM {sliclquestions_'.$this->response_table.'} '.
    ...
    private function get_response_text_results($rids = false) {
        ...
        $sql = 'SELECT T.id, T.response, R.submitted AS submitted, R.username, U.username AS username, '.
                ...
                'FROM {sliclquestions_'.$this->response_table.'} T, '.
                '{sliclquestions_response} R, '.
                '{user} U '.
    ...
    private function get_response_single_results($rids=false) {
        ...
        $sql = 'SELECT rt.id, qc.id as cid, qc.content '.
               'FROM {sliclquestions_quest_choice} qc, '.
               '{sliclquestions_'.$this->response_table.'} rt '.
        ...
        $sql = 'SELECT rt.id, rt.response, qc.content '.
               'FROM {sliclquestions_response_other} rt, '.
                    '{sliclquestions_quest_choice} qc '.
    ...
    private function get_response_rank_results($rids=false) {
        ...
        if ($this->type_id == QUESRATE) {
            ...
            if ($rows = $DB->get_records_select('sliclquestions_quest_choice', $select)) {
                foreach ($rows as $row) {
                    $this->counts[$row->content] = new stdClass();
                    $nbna = $DB->count_records('sliclquestions_response_rank', array('question_id' => $this->id,
            ...
            if (!$isrestricted) {
                if (!empty ($rankvalue)) {
                    ...
                    FROM {$CFG->prefix}sliclquestions_quest_choice c, {$CFG->prefix}sliclquestions_{$this->response_table} r
                ...
                $sql = "SELECT c.id, c.content, a.average, a.num
                        FROM {sliclquestions_quest_choice} c
                        INNER JOIN
                             (SELECT c2.id, AVG(a2.rank+1) AS average, COUNT(a2.response_id) AS num
                              FROM {sliclquestions_quest_choice} c2, {$CFG->prefix}sliclquestions_{$this->response_table} a2
            ...
            $sql = 'SELECT A.rank, COUNT(A.response_id) AS num '.
                   'FROM {sliclquestions_'.$this->response_table.'} A '.
    ...
    private function display_response_bool_results($rids=false) {
        ...
        if ($rows = $this->get_response_bool_results($rids)) {
            ...
            echo '<p class="generaltable">&nbsp;'.get_string('noresponsedata', 'sliclquestions').'</p>';
        }
    }
            ...
            foreach ($rows as $idx => $row) {
                if (strpos($idx, 'other') === 0) {
                    ...
                    $content = preg_replace(array('/^!other=/', '/^!other/'),
                            array('', get_string('other', 'sliclquestions')), $ccontent);
                    $content .= ' ' . clean_text($answer);
                    $textidx = $content;
                    $this->counts[$textidx] = !empty($this->counts[$textidx]) ? ($this->counts[$textidx] + 1) : 1;
                } else {
                    $contents = sliclquestions_choice_values($row->content);
                ...
                if (array_key_exists($ccontent, $rows)) {
                    ...
                    if (isset($rows[$ccontent]->averagevalue)) {
                        ...
                        if ($stravgvalue == '' && !$osgood) {
                            $stravgvalue = ' ('.get_string('andaveragevalues', 'sliclquestions').')';
                        }
                    } else {
                        $avgvalue = null;
                    }
    ...
    private function question_display($formdata, $descendantsdata, $qnum='', $blanksliclquestions) {
        global $qtypenames;

        $method = $qtypenames[$this->type_id].'_survey_display';
        if (method_exists($this, $method)) {
            $this->questionstart_survey_display($qnum, $formdata, $descendantsdata);
            $this->$method($formdata, $descendantsdata, $blanksliclquestions);
    ...
    public function survey_display($formdata, $descendantsdata, $qnum='', $blanksliclquestions=false) {
        $this->question_display($formdata, $descendantsdata, $qnum, $blanksliclquestions);
    }

    public function questionstart_survey_display($qnum, $formdata='') {
        global $OUTPUT, $SESSION, $sliclquestions, $PAGE;
        $currenttab = $SESSION->sliclquestions->current_tab;
        $pagetype = $PAGE->pagetype;
        $skippedquestion = false;
        $skippedclass = '';
        $autonum = $sliclquestions->autonum;
        ...
        // If we are on report page and this sliclquestions has dependquestions and this question was skipped.
        ...
        // In report mode, If sliclquestions is set to no numbering,
        // also hide answers to questions that have not been answered.
        $displayclass = 'qn-container';
        if ($pagetype == 'mod-sliclquestions-preview' || ($nonumbering
        ...
        if ($this->type_id != SLICLQUESSECTIONTEXT) {
            if (!$nonumbering) {
                ...
                echo get_string('questionnum', 'sliclquestions');
            ...
            if ($this->required == 'y') {
                ...
                $required .= get_string('required', 'sliclquestions');
                $required .= html_writer::end_tag('div');
                $required .= html_writer::empty_tag('img',
                        array('class' => 'req',
                                'title' => get_string('required', 'sliclquestions'),
                                'alt' => get_string('required', 'sliclquestions'),
    ...
    private function yesno_survey_display($data, $descendantsdata, $blanksliclquestions=false) {
        ...
        if ($this->required == 'n') {
            ...
            if (!$ischecked && !$blanksliclquestions) {
                $output .= ' checked="checked"';
            }
            if ($onclickdepend) {
                $output .= ' onclick="depend(\''.$descendants.'\', \'\')"';
            }
            $content = get_string('noanswer', 'sliclquestions');
    ...
    private function essay_survey_display($data) { // Essay.
        ...
        if ($canusehtmleditor) {
            ...
            $editor->use_editor($name, sliclquestions_get_editor_options($this->context));
    ...
    private function radio_survey_display($data, $descendantsdata, $blanksliclquestions=false) { // Radio buttons
        global $idcounter;  // To make sure all radio buttons have unique ids. // JR 20 NOV 2007.
        ...
        echo $output;
        if ($otherempty) {
            sliclquestions_notify (get_string('otherempty', 'sliclquestions'));
        }
    }
        ...
        if (!empty($data) ) {
            ...
            if ( $data->{'q'.$this->id} ) {
                ...
                if ($nbboxes < $min || $nbboxes > $max) {
                    $msg = get_string('boxesnbreq', 'sliclquestions');
                    if ($min == $max) {
                        $msg .= '&nbsp;'.get_string('boxesnbexact', 'sliclquestions', $min);
                    } else {
                        if ($min && ($nbboxes < $min)) {
                            $msg .= get_string('boxesnbmin', 'sliclquestions', $min);
                            if ($nbboxes > $max) {
                                $msg .= ' & ' .get_string('boxesnbmax', 'sliclquestions', $max);
                            }
                        } else {
                            if ($nbboxes > $max ) {
                                $msg .= get_string('boxesnbmax', 'sliclquestions', $max);
    ...
    private function rate_survey_display($data, $descendantsdata='', $blanksliclquestions=false) {
        $disabled = '';
        if ($blanksliclquestions) {
        ...
        if ($nbchoices > 1 && $this->precise != 2 && !$blanksliclquestions) {
            echo '<td></td>';
        }
        ...
        if ( ($num != $nbchoices) && ($num != 0) ) {
            sliclquestions_notify(get_string('checkallradiobuttons', 'sliclquestions', $nbchoices));
            $notcomplete = true;
        }
        ...
        foreach ($this->choices as $cid => $choice) {
            if (isset($choice->content)) {
                ...
                for ($j = 0; $j < $this->length + $isna; $j++) {
                    ...
                    $i = $j + 1;
                    echo html_writer::tag('span', get_string('option', 'sliclquestions', $i),
    ...
    private function date_survey_display($data) { // Date.
        ...
        $datemess .= get_string('dateformatting', 'sliclquestions');
        $datemess .= html_writer::end_tag('div');
        if (!empty($data->{'q'.$this->id})) {
            $dateentered = $data->{'q'.$this->id};
            $setdate = sliclquestions_check_date ($dateentered, false);
            if ($setdate == 'wrongdateformat') {
                $msg = get_string('wrongdateformat', 'sliclquestions', $dateentered);
                sliclquestions_notify($msg);
            } else if ($setdate == 'wrongdaterange') {
                $msg = get_string('wrongdaterange', 'sliclquestions');
                sliclquestions_notify($msg);
            } else {
        ...
        if (isset($data->{'q'.$this->id})) {
            ...
            if ($mynumber != '') {
                ...
                if (!is_numeric($mynumber) ) {
                    $msg = get_string('notanumber', 'sliclquestions', $mynumber);
                    sliclquestions_notify ($msg);
                } else {
                    if ($precision) {
                        $pos = strpos($mynumber, '.');
                    ...
                    if ( $mynumber != $mynumber0) {
                        ...
                        $msg = get_string('numberfloat', 'sliclquestions', $a);
                        sliclquestions_notify ($msg);
                    }
                }
            }
    ...
    public function rate_response_display($data) {
        ...
        if ($this->precise == 1) {
            echo '<td style="width:'.$colwidth.'; text-align:center" class="'.$bg.'">'.
                get_string('notapplicable', 'sliclquestions').'</td>';
        }
        if ($osgood) {
    ...
    private function mkrespercent($total, $precision, $showtotals, $sort) {
        ...
        $strtotal = get_string('total', 'sliclquestions');
        ...
        $table->head = array_merge($table->head, array(get_string('response', 'sliclquestions'),
                       get_string('average', 'sliclquestions'), get_string('total', 'sliclquestions')));
        ...
        if (!empty($this->counts) && is_array($this->counts)) {
            ...
            $tabledata = array();
            $tabledata = array_merge($tabledata, array('', get_string('noresponsedata', 'sliclquestions')));
            $table->data[] = $tabledata;
        }
    ...
    private function mkreslist($total, $precision, $showtotals) {
        ...
        $strresponse = get_string('response', 'sliclquestions');
        $strnum = get_string('num', 'sliclquestions');
        $table = new html_table();
        ...
        if (!empty($this->counts) && is_array($this->counts)) {
            ...
            $table->data[] = array('', get_string('noresponsedata', 'sliclquestions'));
        }
    ...
    private function mkreslisttext($rows) {
        global $CFG, $SESSION, $sliclquestions, $OUTPUT, $DB;
        $strresponse = get_string('response', 'sliclquestions');
        $viewsingleresponse = $sliclquestions->capabilities->viewsingleresponse;
        $nonanonymous = $sliclquestions->respondenttype != 'anonymous';
        $table = new html_table();
        if ($viewsingleresponse && $nonanonymous) {
            $strrespondent = get_string('respondent', 'sliclquestions');
            $table->align = array('left', 'left');
            $currentgroupid = '';
            if (isset($SESSION->sliclquestions->currentgroupid)) {
                $currentgroupid = $SESSION->sliclquestions->currentgroupid;
            }
            $url = $CFG->wwwroot.'/mod/sliclquestions/report.php?action=vresp&amp;sid='.$sliclquestions->survey->id.
    ...
    private function mkreslistnumeric($total, $precision) {
        ...
        $strresponse = get_string('response', 'sliclquestions');
        $strnum = get_string('num', 'sliclquestions');
        $strnoresponsedata = get_string('noresponsedata', 'sliclquestions');
        $straverage = get_string('average', 'sliclquestions');
        $table = new html_table();
        $table->align = array('left', 'right');
        $table->head = array($strnum, $strresponse);
        $table->size = array('*', '*');
    ...
    private function mkresavg($total, $precision, $showtotals, $length, $sort, $stravgvalue='') {
        global $CFG, $OUTPUT;
        $stravgrank = get_string('averagerank', 'sliclquestions');
        $osgood = false;
        if ($precision == 3) { // Osgood's semantic differential.
            $osgood = true;
            $stravgrank = get_string('averageposition', 'sliclquestions');
        }
        ...
        if ($isna) {
            $isnahead = get_string('notapplicable', 'sliclquestions');
        }
        $table = new html_table();
    ...
    private function mkrescount($rids, $rows, $precision, $length, $sort) {
        ...
        $questionid = $this->id;
        $sql = 'SELECT r.id, c.content, r.rank, c.id AS choiceid '.
                'FROM '.$CFG->prefix.'sliclquestions_quest_choice c , '.
                $CFG->prefix.'sliclquestions_response_rank r '.
        ...
        $strresp = '<div style="text-align:center">'.get_string('responses', 'sliclquestions').'</div>';
        $strtotal = '<strong>'.get_string('total', 'sliclquestions').'</strong>';
        $isna = $this->precise == 1;
        $isnahead = '';
        $osgood = false;
        $nbchoices = count ($this->counts);
        ...
        if ($isna) {
            $isnahead = get_string('notapplicable', 'sliclquestions').'<br />(#)';
        }
        if ($precision == 1) {
            $na = get_string('notapplicable', 'sliclquestions');
        } else {
        ...
        $headings = array('<span class="smalltext">'.get_string('responses', 'sliclquestions').'</span>');
        $chartkeys = array();
        if ($osgood) {
            $align = array('right');
        } else {
        ...
        if ($isrestricted) {
            array_push($headings, get_string('notapplicable', 'sliclquestions'));
            array_push($align, 'center');
        }
        array_push($align, 'center');
        if ($na) {
