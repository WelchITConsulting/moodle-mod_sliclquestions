...
class sliclquestions {
     ...
     *
     */
    public function __construct($id = 0, $sliclquestions = null, &$course, &$cm, $addquestions = true) {
        global $DB;
        ...
        $this->course = $course;
        $this->cm = $cm;
        // When we are creating a brand new sliclquestions, we will not yet have a context.
        ...
        // Load the capabilities for this user and sliclquestions, if not creating a new one.
        if (!empty($this->cm->id)) {
    ...
    public function add_survey($sid = 0, $survey = null) {
        ...
        if ($sid) {
            $this->survey = $DB->get_record('sliclquestions_survey', array('id' => $sid));
    ...
    public function view() {
        ...
        $PAGE->requires->js_init_call('M.mod_sliclquestions.init_attempt_form', null, false, sliclquestions_get_js_module());
        ...
        if (!$this->capabilities->view) {
            echo('<br/>');
            sliclquestions_notify(get_string("guestsno", "sliclquestions", $this->name));
        ...
        if (!$this->is_active()) {
            ...
            .get_string('notavail', 'sliclquestions')
            .'</div>';
        } else if (!$this->is_open()) {
                echo '<div class="notifyproblem">'
                .get_string('notopen', 'sliclquestions', userdate($this->opendate))
                .'</div>';
        } else if ($this->is_closed()) {
            echo '<div class="notifyproblem">'
            .get_string('closed', 'sliclquestions', userdate($this->closedate))
            .'</div>';
        } else if (!$this->user_is_eligible($USER->id)) {
            echo '<div class="notifyproblem">'
            .get_string('noteligible', 'sliclquestions')
            .'</div>';
            ...
            if (data_submitted() && confirm_sesskey() && isset($viewform->submit) && isset($viewform->submittype) &&
                ...
                sliclquestions_record_submission($this, $USER->id, $rid);

                if ($this->grade != 0) {
                    $sliclquestions = new Object();
                    $sliclquestions->id = $this->id;
                    $sliclquestions->name = $this->name;
                    $sliclquestions->grade = $this->grade;
                    $sliclquestions->cmidnumber = $this->cm->idnumber;
                    $sliclquestions->courseid = $this->course->id;
                    sliclquestions_update_grades($sliclquestions, $quser);
                }
                ...
                add_to_log($this->course->id, "sliclquestions", "submit", "view.php?id={$this->cm->id}", "{$this->name}",
                    $this->cm->id, $USER->id);
            ...
            echo ('<div class="notifyproblem">'.get_string("alreadyfilled", "sliclquestions", $msgstring).'</div>');
        }
    ...
    *
    */
    public function view_response($rid, $referer= '', $blanksliclquestions = false, $resps = '', $compare = false,
        ...
        if ($referer != 'print') {
            ...
            if ($feedbackmessages) {
                echo $OUTPUT->heading(get_string('feedbackreport', 'sliclquestions'), 3);
            ...
            if ($this->survey->feedbacknotes) {
                $text = file_rewrite_pluginfile_urls($this->survey->feedbacknotes, 'pluginfile.php',
                                $this->context->id, 'mod_sliclquestions', 'feedbacknotes', $this->survey->id);
    ...
    */
    public function view_all_responses($resps) {
        ...
        if ($resps) {
            ...
            echo (get_string('noresponses', 'sliclquestions'));
        }
    ...
    public function user_time_for_new_attempt($userid) {
        ...
        if (!($attempts = $DB->get_records_select('sliclquestions_attempts', $select, null, 'timemodified DESC'))) {
            return true;
        }
    ...
    public function can_view_response($rid) {
        ...
        if (!empty($rid)) {
            $response = $DB->get_record('sliclquestions_response', array('id' => $rid));
    ...
    public function count_submissions($userid=false) {
        ...
        if (!$userid) {
            ...
            return $DB->count_records('sliclquestions_response', array('survey_id' => $this->sid, 'complete' => 'y'));
        } else {
            return $DB->count_records('sliclquestions_response', array('survey_id' => $this->sid, 'username' => $userid,
                                      'complete' => 'y'));
        }
    }
    ...
    public function print_survey($userid=false, $quser) {
        ...
        // If student saved a "resume" sliclquestions OR left a sliclquestions unfinished
        ...
        // Save each section 's $formdata somewhere in case user returns to that page when navigating the sliclquestions.
        if (!empty($formdata->next)) {
            ...
            if ( $msg ) {
                ...
                // Skip logic.
                $formdata->sec++;
                if (sliclquestions_has_dependencies($this->questions)) {
                    $nbquestionsonpage = sliclquestions_nb_questions_on_page($this->questions,
                                    $this->questionsbysec[$formdata->sec], $formdata->rid);
                    while (count($nbquestionsonpage) == 0) {
                        $this->response_delete($formdata->rid, $formdata->sec);
                        $formdata->sec++;
                        // We have reached the end of sliclquestions on a page without any question left.
                        if ($formdata->sec > $numsections) {
                            $SESSION->sliclquestions->end = true; // End of sliclquestions reached on a no questions page.
                            break;
                        }
                        $nbquestionsonpage = sliclquestions_nb_questions_on_page($this->questions,
                                        $this->questionsbysec[$formdata->sec], $formdata->rid);
                    }
                    $SESSION->sliclquestions->nbquestionsonpage = $nbquestionsonpage;
                }
            }
        }
        ...
        if (!empty($formdata->prev)) {
            ...
            // unlock sliclquestions->end to allow navigate back to previous page.
            if (isset($SESSION->sliclquestions->end) && $SESSION->sliclquestions->end == true) {
                $SESSION->sliclquestions->end = false;
                $formdata->sec --;
            }
        ...
        if (isset($this->questions) && $numsections) { // Sanity check.
            ...
            if ($formdata->sec > 1) {
                echo '<input type="submit" name="prev" value="<<&nbsp;'.get_string('previouspage', 'sliclquestions').'" />';
            }
            if ($this->resume) {
                echo '<input type="submit" name="resume" value="'.get_string('save', 'sliclquestions').'" />';
            }
            ...
            if ($formdata->sec == $numsections) {
                echo '
                    ...
                    <input type="submit" name="submit" value="'.get_string('submitsurvey', 'sliclquestions').'" /></div>';
            } else {
                echo '&nbsp;<div><input type="submit" name="next" value="'.
                                get_string('nextpage', 'sliclquestions').'&nbsp;>>" /></div>';
            }
    ...
    private function survey_render($section = 1, $message = '', &$formdata) {
        ...
        if ($section > $numsections) {
            ...
            echo '<div class=warning>'.get_string('finished', 'sliclquestions').'</div>';
    ...
    private function print_survey_start($message, $section, $numsections, $hasrequired, $rid='', $blanksliclquestions=false) {
        ...
        if ($rid) {
            ...
            if ($resp = $DB->get_record('sliclquestions_response', array('id' => $rid)) ) {
                if ($this->respondenttype == 'fullname') {
                    ...
                    // Display name of group(s) that student belongs to... if sliclquestions is set to Groups separate or visible.
        ...
        if ($resp && !$blanksliclquestions) {
            ...
            if ($this->respondenttype == 'anonymous') {
                $ruser = '- '.get_string('anonymous', 'sliclquestions').' -';
            } else {
        ...
        if ($ruser) {
            echo (get_string('respondent', 'sliclquestions').': <strong>'.$ruser.'</strong>');
            if ($this->survey->realm == 'public') {
                // For a public sliclquestions, look for the course that used it.
                $coursename = '';
                $sql = 'SELECT q.id, q.course, c.fullname '.
                       'FROM {sliclquestions} q, {sliclquestions_attempts} qa, {course} c '.
        ...
        if ($this->capabilities->printblank && $blanksliclquestions && $section == 1) {
            // Open print friendly as popup window.
            $imageurl = $CFG->wwwroot.'/mod/sliclquestions/images/';
            $linkname = '<img src="'.$imageurl.'print.gif" alt="Printer-friendly version" />';
            $title = get_string('printblanktooltip', 'sliclquestions');
            $url = '/mod/sliclquestions/print.php?qid='.$this->id.'&amp;rid=0&amp;'.'courseid='.$this->course->id.'&amp;sec=1';
        ...
        if ($section == 1) {
            ...
            if ($this->survey->info) {
                $infotext = file_rewrite_pluginfile_urls($this->survey->info, 'pluginfile.php',
                                $this->context->id, 'mod_sliclquestions', 'info', $this->survey->id);
    ...
    private function print_survey_end($section, $numsections) {
        ...
        if ($numsections > 1) {
            ...
            echo get_string('pageof', 'sliclquestions', $a).'&nbsp;&nbsp;';
            echo '</div>';
        }
    }

    // Blanksliclquestions : if we are printing a blank sliclquestions.
    public function survey_print_render($message = '', $referer='', $courseid, $rid=0, $blanksliclquestions=false) {
        ...
        if (!empty($rid)) {
            ...
            $this->view_response($rid, $referer, $blanksliclquestions);
            return;
        }
        ...
        $action = $CFG->wwwroot.'/mod/sliclquestions/preview.php?id='.$this->cm->id;
        ...
        if ($referer == 'preview' && sliclquestions_has_dependencies($this->questions) ) {
                $descendantsandchoices = sliclquestions_get_descendants_and_choices($this->questions);
        }
        if ($errors == 0) {
            echo '<div class="message">'.get_string('submitpreviewcorrect', 'sliclquestions').'</div>';
        }

        $page = 1;
        foreach ($this->questionsbysec as $section) {
            if ($numsections > 1) {
                echo ('<div class="surveyPage">'.get_string('page', 'sliclquestions').' '.$page.'</div>');
                $page++;
            }
            foreach ($section as $question) {
                ...
                $question->survey_display($formdata, $descendantsdata, $i++, $usehtmleditor = null, $blanksliclquestions, $referer);
            }
        }
        // End of questions.
        if ($referer == 'preview' && !$blanksliclquestions) {
            $url = $CFG->wwwroot.'/mod/sliclquestions/preview.php?id='.$this->cm->id;
            echo '
                    <div>
                        <input type="submit" name="submit" value="'.get_string('submitpreview', 'sliclquestions').'" />
    ...
    public function survey_update($sdata) {
        ...
        if (empty($this->survey->id)) {
            ...
            $this->survey->id = $DB->insert_record('sliclquestions_survey', $record);
            $this->add_survey($this->survey->id);

            if (!$this->survey->id) {
                $errstr = get_string('errnewname', 'sliclquestions') .' [ :  ]'; // TODO: notused!
                return(false);
            }
        } else {
            ...
            $name = $DB->get_field('sliclquestions_survey', 'name', array('id' => $this->survey->id));

            // Trying to change survey name.
            if (trim($name) != trim(stripslashes($sdata->name))) {  // $sdata will already have slashes added to it.
                $count = $DB->count_records('sliclquestions_survey', array('name' => $sdata->name));
                if ($count != 0) {
                    $errstr = get_string('errnewname', 'sliclquestions');  // TODO: notused!
                    return(false);
                }
            ...
            $result = $DB->update_record('sliclquestions_survey', $surveyrecord);
            if (!$result) {
                $errstr = get_string('warning', 'sliclquestions').' [ :  ]';  // TODO: notused!
                return(false);
            }
        }
    ...
    public function survey_copy($owner) {
        ...
        $i = 0;
        $name = $survey->name;
        while ($DB->count_records('sliclquestions_survey', array('name' => $name)) > 0) {
            $name = $survey->name.(++$i);
        }
        if ($i) {
            $survey->name .= $i;
        ...
        // Create new survey.
        if (!($newsid = $DB->insert_record('sliclquestions_survey', $survey))) {
            return(false);
        }
        ...
        foreach ($this->questions as $question) {
            ...
            foreach ($question->choices as $key => $choice) {
                ...
                if (!$newcid = $DB->insert_record('sliclquestions_quest_choice', $choice)) {
                    return(false);
                }
        ...
        if ($newquestions = $DB->get_records('sliclquestions_question', array('survey_id' => $newsid), 'id')) {
            foreach ($newquestions as $question) {
                if ($question->dependquestion != 0) {
                    ...
                    $DB->update_record('sliclquestions_question', $record);
                }
            }
        }
    ...
    public function type_has_choices() {
        ...
        if ($records = $DB->get_records('sliclquestions_question_type', array(), 'typeid', 'typeid, has_choices')) {
            foreach ($records as $record) {
    ...
    private function response_check_format($section, $formdata, $checkmissing = true, $checkwrongformat = true) {
        ...
        foreach ($this->questionsbysec[$section] as $question) {
            ...
            switch ($tid) {
                ...
                case QUESDATE: // Date.
                    ...
                    $checkdateresult = '';
                    if ($formdata->{'q'.$qid} != '') {
                        $checkdateresult = sliclquestions_check_date($formdata->{'q'.$qid});
                    }
        ...
        if ($checkmissing && $missing) {
            ...
            if ($missing == 1) {
                $message = get_string('missingquestion', 'sliclquestions').$strmissing;
            } else {
                $message = get_string('missingquestions', 'sliclquestions').$strmissing;
            }
            if ($wrongformat) {
                $message .= '<br />';
            }
        ...
        if ($checkwrongformat && $wrongformat) {
            if ($nonumbering) {
                $message .= get_string('wronganswers', 'sliclquestions');
            } else {
                if ($wrongformat == 1) {
                    $message .= get_string('wrongformat', 'sliclquestions').$strwrongformat;
                } else {
                    $message .= get_string('wrongformats', 'sliclquestions').$strwrongformat;
                }
            }
        }
        return ($message);
    ...
    private function response_delete($rid, $sec = null) {
        ...
        foreach (array('response_bool', 'resp_single', 'resp_multiple', 'response_rank', 'response_text',
            ...
            $DB->delete_records_select('sliclquestions_'.$tbl, $select);
        }
    }
    ...
    private function get_response($username, $rid = 0) {
        ...
        if ($rid != 0) {
            ...
            if ($records = $DB->get_records_select('sliclquestions_response', $select, null, 'submitted DESC',
                                              'id,survey_id', 0, 1)) {
                $rec = reset($records);
                return $rec->id;
            } else {
    ...
    private function response_select_max_sec($rid) {
        ...
        $max = $DB->count_records_select('sliclquestions_question', $select) + 1;
    ...
    private function response_select_max_pos($rid) {
        ...
        foreach (array('response_bool', 'resp_single', 'resp_multiple', 'response_rank', 'response_text',
                       'response_other', 'response_date') as $tbl) {
            $sql = 'SELECT MAX(q.position) as num FROM {sliclquestions_'.$tbl.'} a, {sliclquestions_question} q '.
        ...
        while (list($qid, $arr) = each($res)) {
            ...
            if (strstr($qid, '_')) {
                if ($qtype == QUESRADIO) {     // Single.
                    $nam[$qpos][$qname.'_'.get_string('other', 'sliclquestions')] = $q4;
                    continue;
                }
                // Multiple OR rank.
                if ($oldpos != $qpos) {
                ...
                if ($qtype == QUESRATE) {     // Rate.
                    $qname .= "->$qchoice";
                    if ($q4 == -1) {
                        // Here $q4 = get_string('notapplicable', 'sliclquestions'); DEV JR choose one solution please.
                        $q4 = '';
                    } else {
                        if (is_numeric($q4)) {
                            $q4++;
    ...
    private function response_send_email($rid, $userid=false) {
        ...
        $name = s($this->name);
        if ($record = $DB->get_record('sliclquestions_survey', array('id' => $this->survey->id))) {
            $email = $record->email;
        } else {
            $email = '';
        }
        ...
        $url = $CFG->wwwroot.'/mod/sliclquestions/report.php?action=vresp&amp;sid='.$this->survey->id.
                '&amp;rid='.$rid.'&amp;instance='.$this->id;

        // Html and plaintext body.
        $bodyhtml        = '<a href="'.$url.'">'.$url.'</a>'.$endhtml;
        $bodyplaintext   = $url.$endplaintext;
        $bodyhtml       .= get_string('surveyresponse', 'sliclquestions') .' "'.$name.'"'.$endhtml;
        $bodyplaintext  .= get_string('surveyresponse', 'sliclquestions') .' "'.$name.'"'.$endplaintext;
    ...
    private function response_insert($sid, $section, $rid, $userid, $resume=false) {
        ...
        if ($resume) {
            add_to_log($this->course->id, "sliclquestions", "save", "view.php?id={$this->cm->id}",
    ...
    private function response_select($rid, $col = null, $csvexport = false, $choicecodes=0, $choicetext=1) {
        ...
        $sid = $this->survey->id;
        $values = array();
        $stringother = get_string('other', 'sliclquestions');
        if ($col == null) {
            $col = '';
        }
        ...
        $sql = 'SELECT q.id '.$col.', a.choice_id '.
               'FROM {sliclquestions_response_bool} a, {sliclquestions_question} q '.
        ...
        $sql = 'SELECT q.id '.$col.', q.type_id as q_type, c.content as ccontent,c.id as cid '.
               'FROM {sliclquestions_resp_single} a, {sliclquestions_question} q, {sliclquestions_quest_choice} c '.
        ...
        if ($records = $DB->get_records_sql($sql, array($rid))) {
            foreach ($records as $qid => $row) {
                ...
                if ($csvexport) {
                    static $i = 1;
                    $qrecords = $DB->get_records('sliclquestions_quest_choice', array('question_id' => $qid));
                    foreach ($qrecords as $value) {
                        if ($value->id == $cid) {
                            $contents = sliclquestions_choice_values($value->content);
                            if ($contents->modname) {
                                $row->ccontent = $contents->modname;
                            } else {
                                $content = $contents->text;
                                if (preg_match('/^!other/', $content)) {
                                    $row->ccontent = get_string('other', 'sliclquestions');
        ...
        $sql = 'SELECT a.id as aid, q.id as qid '.$col.',c.content as ccontent,c.id as cid '.
               'FROM {sliclquestions_resp_multiple} a, {sliclquestions_question} q, {sliclquestions_quest_choice} c '.
               'WHERE a.response_id = ? AND a.question_id=q.id AND a.choice_id=c.id '.
        ...
        if ($csvexport) {
            ...
            if (!empty($records)) {
                ...
                $sql = 'SELECT * FROM {sliclquestions_quest_choice} WHERE '.$qids2.
                    'ORDER BY id';
                if ($records2 = $DB->get_records_sql($sql)) {
                    foreach ($records2 as $qid => $row2) {
                        ...
                        $otherend = false;
                        if ($c2 == '!other') {
                            $c2 = '!other='.get_string('other', 'sliclquestions');
                        }
                        if (preg_match('/^!other/', $c2)) {
                            $otherend = true;
                        } else {
                            $contents = sliclquestions_choice_values($c2);
                            if ($contents->modname) {
                                $c2 = $contents->modname;
                        ...
                        $sql = 'SELECT a.name as name, a.type_id as q_type, a.position as pos ' .
                                'FROM {sliclquestions_question} a WHERE id = ?';
                        ...
                        if ($otherend) {
                            ...
                            $newrow2[] = $type1;
                            $newrow2[] = $name1;
                            $newrow2[] = '['.get_string('other', 'sliclquestions').']';
                            $newrow2[] = $selected;
                            $tmp2 = $qid2.'_other';
                            $values["$tmp2"] = $newrow2;
                        }
        ...
        $sql = 'SELECT c.id as cid, c.content as content, a.response as aresponse, q.id as qid, q.position as position,
               ...
               'FROM {sliclquestions_response_other} a, {sliclquestions_question} q, {sliclquestions_quest_choice} c '.
               'WHERE a.response_id= ? AND a.question_id=q.id AND a.choice_id=c.id '.
        ...
        $sql = 'SELECT a.id as aid, q.id AS qid, q.precise AS precise, c.id AS cid '.$col.', c.content as ccontent,
                                a.rank as arank '.
               'FROM {sliclquestions_response_rank} a, {sliclquestions_question} q, {sliclquestions_quest_choice} c '.
               'WHERE a.response_id= ? AND a.question_id=q.id AND a.choice_id=c.id '.
                    ...
                    if ($key != 'content') { // No need to keep question text - ony keep choice text and rank.
                        if ($key == 'ccontent') {
                            if ($osgood) {
                                ...
                                $contents = sliclquestions_choice_values($contentleft);
                                if ($contents->title) {
                                    $contentleft = $contents->title;
                                }
                                $contents = sliclquestions_choice_values($contentright);
                                if ($contents->title) {
                                ...
                                $contents = sliclquestions_choice_values($val);
                                if ($contents->modname) {
        ...
        $sql = 'SELECT q.id '.$col.', a.response as aresponse '.
               'FROM {sliclquestions_response_text} a, {sliclquestions_question} q '.
        ...
        // Response_date.
        $sql = 'SELECT q.id '.$col.', a.response as aresponse '.
               'FROM {sliclquestions_response_date} a, {sliclquestions_question} q '.
               'WHERE a.response_id=\''.$rid.'\' AND a.question_id=q.id ';
        if ($records = $DB->get_records_sql($sql)) {
            ...
            foreach ($records as $qid => $row) {
                ...
                foreach ($row as $key => $val) {
                    if (!is_numeric($key)) {
                        $newrow[] = $val;
                        // Convert date from yyyy-mm-dd database format to actual sliclquestions dateformat.
    ...
    private function response_goto_thankyou() {
        ...
        if ($result = $DB->get_record_select('sliclquestions_survey', $select, null, $fields)) {
        ...
        if (empty($thankhead)) {
            $thankhead = get_string('thank_head', 'sliclquestions');
        }
        $message = '<h3>'.$thankhead.'</h3>'.format_text(file_rewrite_pluginfile_urls($thankbody, 'pluginfile.php',
                        $this->context->id, 'mod_sliclquestions', 'thankbody', $this->survey->id), FORMAT_HTML);
        echo ($message);
        ...
        if ($this->capabilities->readownresponses) {
            echo('<a href="'.$CFG->wwwroot.'/mod/sliclquestions/myreport.php?id='.
    ...
    private function response_goto_saved($url) {
        global $CFG;
        $resumesurvey = get_string('resumesurvey', 'sliclquestions');
        $savedprogress = get_string('savedprogress', 'sliclquestions', '<strong>'.$resumesurvey.'</strong>');
    ...
    public function survey_results_navbar_alpha($currrid, $currentgroupid, $cm, $byresponse) {
        global $CFG, $DB, $OUTPUT;
        // Is this sliclquestions set to fullname or anonymous?
        ...
        if ($isfullname) {
            ...
            'FROM '.$CFG->prefix.'sliclquestions_response R,
            '.$CFG->prefix.'user U
            '.$groupmembers.
        ...
        $url = $CFG->wwwroot.'/mod/sliclquestions/report.php?action=vresp&amp;group='.$currentgroupid;
        $linkarr = array();
        if (!$byresponse) {     // Display navbar.
            ...
            if ($nextrid != null) {
                ...
                array_push($linkarr, '<a href="'.$url.'&amp;rid='.$lastrid.'&amp;individualresponse=1"
                                ...
                                get_string('lastrespondent', 'sliclquestions').'</a>&nbsp;<b>>></b>');
            }
            $url = $CFG->wwwroot.'/mod/sliclquestions/report.php?action=vresp&byresponse=1&group='.$currentgroupid;
            // Display navbar.
            echo $OUTPUT->box_start('respondentsnavbar');
            echo implode(' | ', $linkarr);
            echo '<br /><b><<< <a href="'.$url.'">'.get_string('viewbyresponse', 'sliclquestions').'</a></b>';
            ...
            $imageurl = $CFG->wwwroot.'/mod/sliclquestions/images/';
            $linkname = '<img src="'.$imageurl.'print.gif" alt="Printer-friendly version" />';
            $url = '/mod/sliclquestions/print.php?qid='.$this->id.'&amp;rid='.$currrid.
            ...
            for ($i = 0; $i < $total; $i++) {
                if ($isfullname) {
                    ...
                    array_push($linkarr, '<a title = "'.$responsedate.'" href="'.$url.'&amp;rid='.
                        ...
                        get_string('response', 'sliclquestions').($i + 1).'</a>'.'&nbsp;');
                }
            }
    ...
    public function survey_results($precision = 1, $showtotals = 1, $qid = '', $cids = '', $rid = '',
        ...
        $SESSION->sliclquestions->noresponses = false;
        if (empty($precision)) {
            $precision  = 1;
        }
        if ($showtotals === '') {
        ...
        $haschoices = array();
        $responsetable = array();
        if (!($types = $DB->get_records('sliclquestions_question_type', array(), 'typeid', 'typeid, has_choices, response_table'))) {
            $errmsg = sprintf('%s [ %s: question_type ]',
                    get_string('errortable', 'sliclquestions'), 'Table');
            return($errmsg);
        }
        ...
        if (empty($this->survey)) {
            $errmsg = get_string('erroropening', 'sliclquestions') ." [ ID:${sid} R:";
            return($errmsg);
        }

        if (empty($this->questions)) {
            $errmsg = get_string('erroropening', 'sliclquestions') .' '. 'No questions found.';
            return($errmsg);
        }
        ...
        if (!empty($rid)) {
            ...
            if (!($rows = $DB->get_records_sql($sql))) {
                echo (get_string('noresponses', 'sliclquestions'));
                $SESSION->sliclquestions->noresponses = true;
                return;
            }
            $total = count($rows);
            echo (' '.get_string('responses', 'sliclquestions').": <strong>$total</strong>");
            if (empty($rows)) {
                $errmsg = get_string('erroropening', 'sliclquestions') .' '. get_string('noresponsedata', 'sliclquestions');
                    return($errmsg);
            }
    ...
    */
    public function generate_csv($rid='', $userid='', $choicecodes=1, $choicetext=0, $currentgroupid) {
        ...
        $stringother = get_string('other', 'sliclquestions');
        $columns = array(
                get_string('response', 'sliclquestions'),
                get_string('submitted', 'sliclquestions'),
                get_string('institution'),
                get_string('department'),
                get_string('course'),
                get_string('group'),
                get_string('id', 'sliclquestions'),
                get_string('fullname'),
                get_string('username')
            );
        ...
        if (!$survey = $DB->get_record('sliclquestions_survey', array('id' => $this->survey->id))) {
        ...
        if (!($records = $DB->get_records_select('sliclquestions_question', $select, null, 'position', $fields))) {
            $records = array();
        }
        ...
        foreach ($records as $record) {
            ...
            if ($type == QUESRADIO || $type == QUESCHECK || $type == QUESRATE) {
                ...
                "LEFT JOIN {sliclquestions_quest_choice} c ON question_id = q.id ".
        ...
        if ($rid) {             // Send e-mail for a unique response ($rid).
            ...
            if (!($records = $DB->get_records_select('sliclquestions_response', $select, null, 'submitted', $fields))) {
                $records = array();
            }
        ...
        foreach ($records as $record) {
            ...
            $submitted = date(get_string('strfdateformatcsv', 'sliclquestions'), $record->submitted);
            $institution = '';
            $department = '';
            ...
            // Moodle:
            //  Get the course name that this sliclquestions belongs to.
            ...
            if ($isanonymous) {
                $fullname = get_string('anonymous', 'sliclquestions');
                $username = '';
                $uid = '';
            }
            $arr = array();
        ...
        if (is_array($questions)) {
            ...
            foreach ($questions as $question) {
                ...
                if ($question->id == $movequestion->id) {
                    ...
                    $DB->update_record("sliclquestions_question", $movequestion);
                    continue;
                }
                $question->position = $index;
                $DB->update_record("sliclquestions_question", $question);
                $index++;
            }
            return true;
        }
    ...
    public function response_analysis ($rid, $resps, $compare, $isgroupmember, $allresponses, $currentgroupid) {
        ...
        require_once($CFG->dirroot.'/mod/sliclquestions/drawchart.php');
        if ($resp = $DB->get_record('sliclquestions_response', array('id' => $rid)) ) {
            $userid = $resp->username;
            if ($user = $DB->get_record('user', array('id' => $userid))) {
                $ruser = fullname($user);
        ...
        if ($this->survey->feedbackscores) {
            ...
            $table->wrap = array();
            if ($compare) {
                $table->head = array(get_string('feedbacksection', 'sliclquestions'), $ruser, $groupname);
            } else {
                $table->head = array(get_string('feedbacksection', 'sliclquestions'), $groupname);
            }
        }
        ...
        // Find if there are any feedbacks in this sliclquestions.
        $sql = "SELECT * FROM {sliclquestions_fb_sections} WHERE survey_id = $sid AND section IS NOT NULL";
        if (!$fbsections = $DB->get_records_sql($sql)) {
            return null;
        }

        $fbsectionsnb = array_keys($fbsections);
        // Calculate max score per question in sliclquestions.
        $qmax = array();
        $totalscore = 0;
        $maxtotalscore = 0;
        foreach ($questions as $question) {
        ...
        foreach ($rids as $rrid) {
            ...
            if ($responses = $DB->get_records_sql($sql, array($rrid))) {
                ...
                foreach ($responses as $response) {
                    ...
                    $firstcid[$qid] = $DB->get_record('sliclquestions_quest_choice',
                                    array('question_id' => $qid), 'id', IGNORE_MULTIPLE);
                    $firstcidid = $firstcid[$qid]->id;
                    $cidvalue = $firstcidid + $rank;
                    $sql = "SELECT * FROM {sliclquestions_quest_choice} WHERE id = $cidvalue";
        ...
        if ($feedbacksections == 1) {
            ...
            $feedbacks = $DB->get_records('sliclquestions_feedback', array('section_id' => $sectionid));
            $labels = array();
            ...
            $feedback = $DB->get_record_select('sliclquestions_feedback',
            ...
            $sectionheading = file_rewrite_pluginfile_urls($sectionheading, 'pluginfile.php',
                            $this->context->id, 'mod_sliclquestions', 'sectionheading', $sectionid);
            ...
            if (!empty($feedback->feedbacktext)) {
                ...
                $feedbacktext = file_rewrite_pluginfile_urls($feedback->feedbacktext, 'pluginfile.php',
                                $this->context->id, 'mod_sliclquestions', 'feedback', $feedback->id);
            ...
            if ($CFG->sliclquestions_usergraph && $this->survey->chart_type) {
        ...
        for ($section = 1; $section <= $feedbacksections; $section++) {
            ...
            if (!$allresponses) {
                ...
                $formatoptions->noclean = true;
                $sectionheading = file_rewrite_pluginfile_urls($sectionheading, 'pluginfile.php',
                                $this->context->id, 'mod_sliclquestions', 'sectionheading', $imageid);
        ...
        if (isset($CFG->sliclquestions_usergraph) && $CFG->sliclquestions_usergraph && $this->survey->chart_type) {
