...
$SESSION->sliclquestions->current_tab = 'nonrespondents';
...
$sliclquestions = new sliclquestions($sid, $sliclquestions, $course, $cm);
$resume = $sliclquestions->resume;
$fullname = $sliclquestions->respondenttype == 'fullname';
$sid = $sliclquestions->sid;
$url = new moodle_url('/mod/sliclquestions/show_nonrespondents.php', array('id' => $cm->id));
...
require_capability('mod/sliclquestions:viewsingleresponse', $context);

// Anonymous sliclquestions.
if (!$fullname) {
    $nonrespondents = sliclquestions_get_incomplete_users($cm, $sid);
    $countnonrespondents = count($nonrespondents);
    if ($resume) {
        $countstarted = 0;
        $countnotstarted = 0;
        $sql = "SELECT username FROM {sliclquestions_response} WHERE survey_id = $sid AND complete = 'n'";
...
if ($action == 'sendmessage' && !empty($subject) && !empty($message)) {
    ...
    $strsliclquestionss = get_string("modulenameplural", "sliclquestions");

    $htmlmessage = "<body id=\"email\">";

    $link1 = $CFG->wwwroot.'/mod/sliclquestions/view.php?id='.$cm->id;

    $htmlmessage .= '<div class="navbar">'.
    '<a target="_blank" href="'.$link1.'">'.format_string($sliclquestions->name, true).'</a>'.
    '</div>';
    ...
    if (is_array($messageuser)) {
        foreach ($messageuser as $userid) {
            ...
            $eventdata->component        = 'mod_sliclquestions';
        ...
        $url = new moodle_url('/mod/sliclquestions/view.php', array('id' => $cm->id));
        redirect($url, $msg, 4);
        exit;
    }
}
...
$PAGE->navbar->add(get_string('show_nonrespondents', 'sliclquestions'));
...
if ($fullname) {
    ...
    $baseurl = new moodle_url('/mod/sliclquestions/show_nonrespondents.php');
    ...
    if (has_capability('mod/sliclquestions:message', $context)) {
        $tablecolumns[] = 'select';
    ...
    $nonrespondents = sliclquestions_get_incomplete_users($cm, $sid, $usedgroupid);
    $countnonrespondents = count($nonrespondents);
...
$nonrespondents = sliclquestions_get_incomplete_users($cm, $sid, $usedgroupid, $sort, $startpage, $pagecount);
...
if (!$nonrespondents) {
    ...
    echo print_string('non_respondents', 'sliclquestions');
    echo ' ('.$countnonrespondents.')';
    if (!$fullname) {
        echo ' ['.get_string('anonymous', 'sliclquestions').']';
    }
    echo '<form class="mform" action="show_nonrespondents.php" method="post" id="sliclquestions_sendmessageform">';
    if ($fullname) {
        foreach ($nonrespondents as $nonrespondent) {
            ...
            $data[] = $lastaccess;
            if (has_capability('mod/sliclquestions:message', $context)) {
                // If sliclquestions is set to "resume", look for saved (not completed) responses
                // we use the alt attribute of the checkboxes to store the started/not started value!
                $checkboxaltvalue = '';
                if ($resume) {
                    if ($DB->get_record('sliclquestions_response', array('survey_id' => $sid,
                                    'username' => $nonrespondent, 'complete' => 'n')) ) {
                        $data[] = get_string('started', 'sliclquestions');
                        $checkboxaltvalue = 1;
                    } else {
                        $data[] = get_string('not_started', 'sliclquestions');
                        $checkboxaltvalue = 0;
                    }
                }
        ...
        if (has_capability('mod/sliclquestions:message', $context)) {
            ...
            if ($resume) {
                if ($perpage >= $countnonrespondents) {
                    echo '<input type="button" id="checkstarted" value="'.get_string('checkstarted', 'sliclquestions').'" />'."\n";
                    echo '<input type="button" id="checknotstarted" value="'.
                        get_string('checknotstarted', 'sliclquestions').'" />'."\n";
                }
            }
            echo '</div>';
            echo $OUTPUT->box_end();
            if ($action == 'sendmessage' && !is_array($messageuser)) {
                echo $OUTPUT->notification(get_string('nousersselected', 'sliclquestions'));
            }
        }
    } else {// Anonymous sliclquestions.
        if (has_capability('mod/sliclquestions:message', $context)) {
            echo '<fieldset>';
            echo '<legend>'.get_string('send_message_to', 'sliclquestions').'</legend>';
                ...
                echo '<input type="radio" name="selectedanonymous" value="all" id="nonrespondents" '.$checked.' />
                    <label for="all">'.get_string('all', 'sliclquestions').'</label>';
            if ($resume) {
                if ($countstarted > 0) {
                        ...
                        <label for="started">'.get_string('status').': '.
                            get_string('started', 'sliclquestions').' ('.$countstarted.')</label>';
                }
                if ($countnotstarted > 0) {
                    ...
                    <label for="notstarted">'.get_string('status').': '.
                        get_string('not_started', 'sliclquestions').' ('.$countnotstarted.')</label>';
                }
            }
    ...
    if (has_capability('mod/sliclquestions:message', $context)) {
        ...
        echo '<legend class="ftoggler">'.get_string('send_message', 'sliclquestions').'</legend>';
        $id = 'message' . '_id';
        $subjecteditor = '&nbsp;&nbsp;&nbsp;<input type="text" id="sliclquestions_subject" size="65"
            maxlength="255" name="subject" value="'.$subject.'" />';
        $format = '';
            $editor = editors_get_preferred_editor();
            $editor->use_editor($id, sliclquestions_get_editor_options($context));
        ...
        $table->data[] = array( '<strong>'.get_string('subject', 'sliclquestions').'</strong>', $subjecteditor);
        ...
        echo '<div class="buttons">';
        echo '<input type="submit" name="send_message" value="'.get_string('send', 'sliclquestions').'" />';
        echo '</div>';
        echo $OUTPUT->box_end();
        ...
        $module = array('name' => 'mod_sliclquestions', 'fullpath' => '/mod/sliclquestions/module.js');
        $PAGE->requires->js_init_call('M.mod_sliclquestions.init_sendmessage', null, false, $module);
    }
}
echo $OUTPUT->box_end();
