...
if ($instance === false) {
    if (!empty($SESSION->instance)) {
        ...
        print_error('requiredparameter', 'sliclquestions');
    }
}
...
// If you can't view the sliclquestions, or can't view a specified response, error out.
$context = context_module::instance($cm->id);
if (!has_capability('mod/sliclquestions:readallresponseanytime', $context) &&
  !($sliclquestions->capabilities->view && $sliclquestions->can_view_response($rid))) {
...
$url = new moodle_url($CFG->wwwroot.'/mod/sliclquestions/report.php');
if ($instance) {
...
$SESSION->sliclquestions_survey_id = $sid;
...
switch ($action) {
    ...
    case 'dresp':  // Delete individual response? Ask for confirmation.
        ...
        if (empty($sliclquestions->survey)) {
            $id = $sliclquestions->survey;
            notify ("sliclquestions->survey = /$id/");
            print_error('surveynotexists', 'sliclquestions');
        } else if ($sliclquestions->survey->owner != $course->id) {
            print_error('surveyowner', 'sliclquestions');
        } else if (!$rid || !is_numeric($rid)) {
            print_error('invalidresponse', 'sliclquestions');
        } else if (!($resp = $DB->get_record('sliclquestions_response', array('id' => $rid)))) {
        ...
        $msg .= get_string('confirmdelresp', 'sliclquestions', $ruser);
        $msg .= '</div>';
    ...
    case 'delallresp': // Delete all responses? Ask for confirmation.
        ...
        if (!($responses = $DB->get_records_select('sliclquestions_response', $select, null, 'id', 'id'))) {
            return;
        }
        ...
        // Print the tabs.
        $SESSION->sliclquestions->current_tab = 'deleteall';
        include('tabs.php');
        ...
        if ($groupmode == 0) {   // No groups or visible groups.
            $msg .= get_string('confirmdelallresp', 'sliclquestions');
        } else {                 // Separate groups.
            $msg .= get_string('confirmdelgroupresp', 'sliclquestions', $groupname);
        }
        $msg .= '</div>';
    ...
    case 'dvresp': // Delete single response. Do it!
        ...
        if (empty($sliclquestions->survey)) {
            ...
            print_error('invalidresponse', 'sliclquestions');
        } else if (!($response = $DB->get_record('sliclquestions_response', array('id' => $rid)))) {
            print_error('invalidresponserecord', 'sliclquestions');
        }
        ...
        if (sliclquestions_delete_response($response, $sliclquestions)) {
            if ($sliclquestions->respondenttype == 'anonymous') {
                    $ruser = '- '.get_string('anonymous', 'sliclquestions').' -';
            }
            $sql = "SELECT R.id, R.survey_id, R.submitted, R.username
            ...
            if (empty($resps)) {
                $redirection = $CFG->wwwroot.'/mod/sliclquestions/view.php?id='.$cm->id;
            } else {
                $redirection = $CFG->wwwroot.'/mod/sliclquestions/report.php?action=vresp&amp;instance='.
                    $instance.'&amp;byresponse=1';
            }
            redirect($redirection);
        } else {
            error (get_string('couldnotdelresp', 'sliclquestions').$rid.get_string('by', 'sliclquestions').$ruser.'?',
                   $CFG->wwwroot.'/mod/sliclquestions/report.php?action=vresp&amp;sid='.$sid.'&amp;&amp;instance='.
                   $instance.'byresponse=1');
        }
        break;

    case 'dvallresp': // Delete all responses in sliclquestions (or group). Do it!
        ...
        if ($groupmode > 0) {
            ...
            if (empty($resps)) {
                ...
                if ($rid === false) {
                    ...
                    $resp = $DB->get_record('sliclquestions_response', array('id' => $rid));
                }
                if (is_numeric($resp->username)) {
                    if ($user = $DB->get_record('user', array('id' => $resp->username))) {
                        $ruser = fullname($user);
        ...
        if (!empty($resps)) {
            ...
            if (empty($resps)) {
                ...
                $redirection = $CFG->wwwroot.'/mod/sliclquestions/report.php?action=vall&amp;sid='.$sid.'&amp;instance='.$instance;
            }
            redirect($redirection);
        } else {
            error (get_string('couldnotdelresp', 'sliclquestions'),
                   $CFG->wwwroot.'/mod/sliclquestions/report.php?action=vall&amp;sid='.$sid.'&amp;instance='.$instance);
        }
        break;

    case 'dwnpg': // Download page options.

        require_capability('mod/sliclquestions:downloadresponses', $context);
        ...
        // Tab setup.
        if (empty($user)) {
            $SESSION->sliclquestions->current_tab = 'downloadcsv';
        } else {
            $SESSION->sliclquestions->current_tab = 'mydownloadcsv';
        }
            ...
            switch ($currentgroupid) {
                ...
                default:     // Members of a specific group.
                    $groupname = get_string('membersofselectedgroup', 'group').' '.get_string('group').' '.
                        $sliclquestionsgroups[$currentgroupid]->name;
            }
        }
        echo "<br /><br />\n";
        echo $OUTPUT->help_icon('downloadtextformat', 'sliclquestions');
        echo '&nbsp;'.(get_string('downloadtext')).':&nbsp;'.get_string('responses', 'sliclquestions').'&nbsp;'.$groupname;
        echo $OUTPUT->heading(get_string('textdownloadoptions', 'sliclquestions'));
        echo $OUTPUT->box_start();
        echo "<form action=\"{$CFG->wwwroot}/mod/sliclquestions/report.php\" method=\"GET\">\n";
        ...
        echo html_writer::checkbox('choicecodes', 1, true, get_string('includechoicecodes', 'sliclquestions'));
        echo "<br />\n";
        echo html_writer::checkbox('choicetext', 1, true, get_string('includechoicetext', 'sliclquestions'));
        echo "<br />\n";
        echo "<br />\n";
        echo "<input type=\"submit\" name=\"submit\" value=\"".get_string('download', 'sliclquestions')."\" />\n";
        echo "</form>\n";
        echo $OUTPUT->box_end();
    ...
    case 'dcsv': // Download responses data as text (cvs) format.
        ...
        // Use the sliclquestions name as the file name. Clean it and change any non-filename characters to '_'.
        $name = clean_param($sliclquestions->name, PARAM_FILE);
        $name = preg_replace("/[^A-Z0-9]+/i", "_", trim($name));

            $choicecodes = optional_param('choicecodes', '0', PARAM_INT);
            $choicetext  = optional_param('choicetext', '0', PARAM_INT);
            $output = $sliclquestions->generate_csv('', $user, $choicecodes, $choicetext, $currentgroupid);
            // CSV
    ...
    case 'vallarsort':   // View all responses sorted in descending order.
        ...
        $PAGE->set_heading(format_string($course->fullname));
        echo $OUTPUT->header();
        if (!$sliclquestions->capabilities->readallresponses && !$sliclquestions->capabilities->readallresponseanytime) {

            // Should never happen, unless called directly by a snoop.
            print_error('nopermissions', '', '', get_string('viewallresponses', 'sliclquestions'));
        ...
        switch ($action) {
            case 'vallasort':
                $SESSION->sliclquestions->current_tab = 'vallasort';
                break;
            case 'vallarsort':
                $SESSION->sliclquestions->current_tab = 'vallarsort';
                break;
            default:
                $SESSION->sliclquestions->current_tab = 'valldefault';
        }
        include('tabs.php');

        $resps = array();
        // Enable choose_group if there are sliclquestions groups and groupmode is not set to "no groups"
        // and if there are more goups than 1 (or if user can view all groups).
        if (is_array($sliclquestionsgroups) && $groupmode > 0) {
            $groupselect = groups_print_activity_menu($cm, $url->out(), true);
            // Count number of responses in each group.
            $castsql = $DB->sql_cast_char2int('R.username');
            foreach ($sliclquestionsgroups as $group) {
                $sql = "SELECT R.id, GM.id as groupid
                    FROM {sliclquestions_response} R, {groups_members} GM
                    WHERE R.survey_id= ? AND
                          R.complete='y' AND
                          GM.groupid= ? AND " . $castsql . "=GM.userid";
        ...
        if (!empty($resps)) {
            $ret = $sliclquestions->response_analysis($rid = 0, $resps, $compare = false,
                            $isgroupmember = false, $allresponses = true, $currentgroupid);
        }

        echo'<div class = "generalbox">';
        echo (get_string('viewallresponses', 'sliclquestions').'. '.$groupname.'. ');
        $strsort = get_string('order_'.$sort, 'sliclquestions');
        echo $strsort;
        echo $OUTPUT->help_icon('orderresponses', 'sliclquestions');

        $ret = $sliclquestions->survey_results(1, 1, '', '', '', $uid = false, $currentgroupid, $sort);
        echo '</div>';
    ...
    default:
        ...
        // Print the tabs.
        if ($byresponse) {
            $SESSION->sliclquestions->current_tab = 'vrespsummary';
        }
        if ($individualresponse) {
            $SESSION->sliclquestions->current_tab = 'individualresp';
        }
        include('tabs.php');
        ...
        if ($noresponses) {
            echo (get_string('group').' <strong>'.groups_get_group_name($currentgroupid).'</strong>: '.
                get_string('noresponses', 'sliclquestions'));
        } else {
            $groupname = get_string('group').': <strong>'.groups_get_group_name($currentgroupid).'</strong>';
            if ($currentgroupid == 0 ) {
                $groupname = get_string('allparticipants');
            ...
            if ($byresponse) {
                echo $OUTPUT->box_start();
                echo $OUTPUT->help_icon('viewindividualresponse', 'sliclquestions').'&nbsp;';
                echo (get_string('viewindividualresponse', 'sliclquestions').' <strong> : '.$groupname.'</strong>');
                echo $OUTPUT->box_end();
            }
            $sliclquestions->survey_results_navbar_alpha($rid, $currentgroupid, $cm, $byresponse);
            if (!$byresponse) { // Show respondents individual responses.
                $sliclquestions->view_response($rid, $referer = '', $blanksliclquestions = false, $resps, $compare = true,
