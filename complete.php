...
require_capability('mod/sliclquestions:view', $context);

$url = new moodle_url($CFG->wwwroot.'/mod/sliclquestions/complete.php');
if (isset($id)) {
    $url->param('id', $id);
} else {
    $url->param('a', $a);
...
add_to_log($course->id, "sliclquestions", "view", "view.php?id=$cm->id", "$sliclquestions->name", $cm->id, $USER->id);
$sliclquestions->strquestionnaires = get_string("modulenameplural", "sliclquestions");
$sliclquestions->strquestionnaire  = get_string("modulename", "sliclquestions");
